...
from utils import parse_input, handle_add_contact, handle_search_contact, handle_edit_contact, handle_sort_notes_by_tags
...
def main():
    ...
    Initializes the ContactManager and NoteManager and provides a command-line interface for the user.
    """
    ...
    while True:
        try:
            user_input = session.prompt("Enter a command (add/search/edit/remove/sort_notes/exit): ")
            ...
            if command in ["exit", "close"]:
                ...
                handle_sort_notes_by_tags(note_manager)
            elif command == "all":
